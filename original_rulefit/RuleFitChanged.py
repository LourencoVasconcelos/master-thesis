...
class RuleEnsemble():
    ...
    def transform(self, X,coefs=None):
        ...
        if   coefs is None :
            a= np.array([rule.transform(X) for rule in rule_list])
            return a.T
        ...
        if 'r' in self.model_type:
            ...
            if not self.exp_rand_tree_size: # simply fit with constant tree size
                ...
                for i_size in np.arange(len(tree_sizes)):
                    ...
                    self.tree_generator.set_params(max_leaf_nodes=size) #set number of rules of that tree
                    ...
                    self.tree_generator.get_params()['n_estimators']  # number of the tree ?
                    self.tree_generator.fit(np.copy(X, order='C'), np.copy(y, order='C'))  #np.copy to guarantee data isn't changed
